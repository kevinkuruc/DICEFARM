@defcomp climatedynamics begin
    ...
    TotForcing    = Parameter(index=[time])   # Increase in radiative forcing (watts per m2 from 1900)
    ...
    function run_timestep(p, v, d, t)
        ...
        if is_first(t)
            ...
            v.TATM[t] = v.TATM[t-1] + p.c1 * ((p.TotForcing[t] - (p.fco22x/p.t2xco2) * v.TATM[t-1]) - (p.c3 * (v.TATM[t-1] - v.TOCEAN[t-1])))
        end
