...
function initialize_dice_farm(p, start_year, end_year, start_dice_year)

    # Create an instance of FAIR to couple in DICE-FARM components.
    m = create_fair(rcp_scenario="RCP60", start_year=start_year, end_year=end_year)
    ...
    set_param!(m, :farm, :APork,              p[:APork])
    set_param!(m, :farm, :ABeef,              p[:ABeef])
    set_param!(m, :farm, :APoultry,             p[:APoultry])
    ...
    set_param!(m, :welfare, :thetaB,        p[:thetaB])
    set_param!(m, :welfare, :thetaC,        p[:thetaC])
    set_param!(m, :welfare, :thetaP,        p[:thetaP])
    set_param!(m, :welfare, :CowEquiv,      p[:CowEquiv])
    set_param!(m, :welfare, :PigEquiv,      p[:PigEquiv])
    set_param!(m, :welfare, :ChickenEquiv,  p[:ChickenEquiv])
...
function create_dice_farm(;start_year=1765, end_year=2500, start_dice_year=2015, datafile=joinpath(dirname(@__FILE__), "..", "data", "DICE2016ANDFAIRParameters.xlsm"))
    ...
    dice_farm = initialize_dice_farm(dicefarm_parameters, start_year, end_year, start_dice_year)
    run(dice_farm)
